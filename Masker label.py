...
class MainWindow(QMainWindow):
    ...
    def colorierZone(self,x,y) :
        self.image_qt.save("/tmp/colorzone.jpg","PNG",100)
        image= cv2.imread("/tmp/colorzone.jpg")
        ...
        self.image_qt = QImage("/tmp/image.jpg")
        self.pixmap = QPixmap.fromImage(self.image_qt)
        w = self.ImageView.width()
        h=self.ImageView.height()
        self.ImageView.setPixmap(self.pixmap.scaled(w,h,Qt.KeepAspectRatio)) 
...
if __name__ == '__main__':
    ...
    # styleSheetFile =QFile('.Remover.qss')
    # styleSheetFile.open(QFile)
    # styleSheet = QString(QLatin1String(styleSheetFile.readAll()))
    #fichier = open("Remover.qss","r")
    #design = fichier.read()
